...
curl -L https://unpkg.com/htmx.org@2.0.4/dist/htmx.min.js -o static/htmx.min.js
```
...
Wait — the `static/` and `templates/` directories live at the repo root, but `assets.go` is in `internal/server/`. The `go:embed` path is relative to the source file, so this will not work.

The conventional fix: move `assets.go` to the repo root package, or better, use `cmd/portal/` which has access to root-relative paths via the embed from there. The cleanest approach for our layout is to put the embed declarations in a dedicated package:

```
internal/assets/assets.go   ← embed directives here
static/htmx.min.js          ← static assets
...
> `go:embed` paths cannot traverse `../` — they must be within the module root or a subtree below the source file. Using `../../` from `internal/assets/` does work because Go resolves the path relative to the source file and validates it is within the module root. If this causes issues on your Go version, move `static/` and `templates/` into `internal/assets/static/` and `internal/assets/templates/` and use relative paths without `../`.
...
```bash
curl -L https://unpkg.com/htmx-ext-sse@2.2.3/sse.js -o static/htmx-ext-sse.min.js
```
