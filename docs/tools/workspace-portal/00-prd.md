...
2. **Wrong scope for OpenCode instances.** Running a single OC instance from the projects root means all project sessions share one process. Per-project configuration (AGENTS.md, `.agents/skills/`) is resolved correctly via directory context, but the skill cache is warm from the first session and is never invalidated for new projects opened later. Per-project instances would start fresh with the correct skills every time.

3. **No remote VS Code access.** There is no way to open a browser-based VS Code session for a specific project directory from a mobile phone or another machine on the Tailscale network. GitHub Codespaces and Gitpod solve this problem for cloud-hosted repos, but the developer works on local checkouts.

The developer needs a self-hosted, mobile-friendly portal that acts as a launcher and manager for per-directory OpenCode and VS Code sessions, accessible over Tailscale (or any reverse proxy) from any device.
...
- Exposes each session on a dedicated HTTPS port via Tailscale serve (optional)
- Runs as a **launchd service** on macOS (always-on, survives terminal close, restarts on crash)
- Is **Tailscale-agnostic** — network exposure is a pluggable concern; the portal binds to localhost and delegates exposure to whatever the user has configured (Tailscale serve, nginx, Cloudflare Tunnel, etc.)
...
10. As a remote developer, I want every directory row to show an "Open OC" button, so that I can start an OpenCode session for that directory with one tap.
...
15. As a remote developer, I want the "Open OC" button to become a direct link to the session once it is running, so that tapping it again opens the session rather than starting a duplicate.
...
19. As a remote developer, I want each running session entry to show its directory path, type (OC or VS Code), port, and a direct link, so that I can reconnect to a session I already started.
...
36. As a self-hoster, I want the workspaces root, OC binary path, code-server binary path, port ranges, and secrets directory to all be configurable, so that the portal is not tied to any specific machine layout.
...
Maintains in-memory session state, persisted to a JSON state file on every mutation. Assigns ports from configured ranges (OC range, VS Code range) by scanning for the first port not in use (checked via `net.Listen`). On startup, reads the state file and validates each entry by checking the process PID; removes orphans. Exposes: `Start(type, dir) (Session, error)`, `Stop(id) error`, `List() []Session`, `Get(id) (Session, bool)`.
...
**`internal/tailscale`**
Optional integration, gated by `config.tailscale.enabled`. Exposes `Register(port int) (url string, error)` and `Deregister(port int) error`. Shells out to the `tailscale` binary (`tailscale serve --bg --https={port} http://localhost:{port}`). If disabled, `Register` is a no-op returning an empty URL. The session manager calls these hooks around `Runner.Start/Stop` — it does not know whether Tailscale is enabled.
...
tailscale.enabled   bool      (default: false)
tailscale.binary    string    (default: tailscale)
...
- Additional secrets (e.g. OC server password) added to `.secrets/` and wired via config as needed
...
**`internal/tailscale`**
Test with a fake `tailscale` binary (a shell script stub): `Register` calls `tailscale serve --bg --https={port} ...` with correct args; `Deregister` calls the correct clear command; when `enabled=false`, neither command is called.
...
The session manager is injected as an interface in all server tests so OC and code-server processes are never actually spawned.
...
- **OpenCode session isolation** beyond process-per-directory. OC's internal multi-session behaviour (multiple chat sessions within one OC process) is not managed by the portal.
...
- The Tailscale integration shells out to the `tailscale` CLI rather than using the Tailscale Go SDK, to avoid adding the SDK as a dependency and to keep the integration loosely coupled.
