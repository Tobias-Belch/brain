...
import { cm, mm, type Length } from "@fea-lib/jscad";
...
const bottomRailHolesOffsetZ = cm(3 + cm(drillWallThickness).value);
...
function normaliseUnits(value: Length): number {
  return mm(value).value; // Convert to scene units (1 scene unit = 1 mm)
}
