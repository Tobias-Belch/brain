...
function WallMounts({
  ...
  const bottomRailHeight = cm(
    cm(dimensions.bestaTop.height).value -
      2 * cm(dimensions.valance.height).value,
  );
  ...
  const extendedDepth = cm(cm(depth).value + cm(dimensions.wallMountExtension.depth).value);
  ...
  const vances = [
    translate(
      ...
      Board({
        width: cm(3 * cm(dimensions.bestaTop.width).value),
...
function normaliseUnits(value: Length): number {
  return cm(value).value; // Convert to scene units (1 scene unit = 1 cm)
}
